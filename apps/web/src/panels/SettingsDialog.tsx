...
export const SettingsDialog = ({ open, onClose }: SettingsDialogProps): React.ReactElement => {
  ...
  return (
    <Modal
      ...
      footer={<><Button variant="ghost" onClick={onClose}>{zh ? '取消' : 'Cancel'}</Button><Button variant="primary" className={css.saveButton} disabled={busy || !draft} onClick={() => void save()}>{busy ? '…' : zh ? '保存设置' : 'Save settings'}</Button></>}
    >
        ...
        {loading || !draft ? <div className={css.note}>{zh ? '正在读取本地配置…' : 'Loading local settings…'}</div> : tab === 'llm' ? (
          <section className={css.section}>
            ...
            <div className={css.grid}>
              ...
              {draft.llm.apiKeyConfigured && <label className={css.compactToggle}><span>{zh ? '清除已保存密钥' : 'Clear saved key'}</span><input className={css.switchInput} type="checkbox" checked={clearLlmKey} onChange={(event) => setClearLlmKey(event.target.checked)} /></label>}
            ...
            <label className={css.toggleRow}><span className={css.toggleCopy}><strong>{zh ? '流式输出' : 'Streaming output'}</strong><small>{zh ? '支持文本调用的供应商增量输出；结构化决策会先完成契约校验再展示。' : 'Stream incremental text on supported calls; structured decisions remain buffered until contract validation.'}</small></span><input className={css.switchInput} type="checkbox" checked={draft.llm.streaming} onChange={(event) => editLlm({ streaming: event.target.checked })} /></label>
            <label className={css.toggleRow}><span className={css.toggleCopy}><strong>{zh ? '打字机效果' : 'Typewriter rendering'}</strong><small>{zh ? '新回答逐字显示；减少动态效果偏好会自动禁用。' : 'Reveal new replies progressively; reduced-motion preferences disable it.'}</small></span><input className={css.switchInput} type="checkbox" checked={draft.llm.typewriter} onChange={(event) => editLlm({ typewriter: event.target.checked })} /></label>
          </section>
        ) : (
          <section className={css.section}>
            ...
            <label className={css.toggleRow}><span className={css.toggleCopy}><strong>{zh ? '启用嵌入配置' : 'Enable embedding configuration'}</strong><small>{zh ? '启用表示配置可供 Agent 使用，不代表立即发起请求。' : 'Makes the configuration available to the Agent; it does not send a request.'}</small></span><input className={css.switchInput} type="checkbox" checked={draft.embedding.enabled} onChange={(event) => editEmbedding({ enabled: event.target.checked })} /></label>
            <div className={css.grid}>
              ...
              {draft.embedding.apiKeyConfigured && <label className={css.compactToggle}><span>{zh ? '清除已保存密钥' : 'Clear saved key'}</span><input className={css.switchInput} type="checkbox" checked={clearEmbeddingKey} onChange={(event) => setClearEmbeddingKey(event.target.checked)} /></label>}
            </div>
          </section>
        )}
      </div>
